        ...
        ...new Set(
          messages
            .map(msg => {
              ...
              if (typeof sender === "object") return sender._id || sender.id;
              return sender;
            })
            .filter(Boolean)
        ),
      ...
      try {
        ...
        const profiles = await Promise.all(
          senderIds.map(async id => {
            ...
            return { id, profile: await res.json() };
          })
        );
  ...
  const recipientProfileImg =
    recipientProfile?.role === "employer" && recipientProfile.companyLogo
      ? recipientProfile.companyLogo.startsWith("http")
        ...
        : `${import.meta.env.VITE_API_BASE_URL}/${recipientProfile.companyLogo}`
    ...
    <div className="w-full bg-white rounded-xl border border-gray-300 p-4 flex flex-col h-full">
        ...
        {recipientProfile && (
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500">{recipientProfile.email}</p>
            </div>
          </div>
        )}
      </div>
      <div className="flex-1 overflow-y-auto mb-4 px-2 custom-scroll">
        {messages.map((message) => {
          ...
          const profileImage =
            isEmployer && senderProfile?.companyLogo
              ? senderProfile.companyLogo.startsWith("http")
                ? senderProfile.companyLogo
                : `${import.meta.env.VITE_API_BASE_URL}/${senderProfile.companyLogo}`
              : senderProfile?.onboarding?.profileImage
              ? `${import.meta.env.VITE_API_BASE_URL}/${senderProfile.onboarding.profileImage}`
              : "/default-avatar.png";
